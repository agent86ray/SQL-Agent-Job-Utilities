...
BEGIN
	...
	TRUNCATE TABLE [dbo].[QUEUE_JOBS_TO_LAUNCH_STAGING];

	--
	-- stage the job steps completed since the last run
	-- rename this table?
	--
	INSERT [dbo].[QUEUE_JOBS_TO_LAUNCH_STAGING] (
		[INSTANCE_ID] 
	,	[JOB_ID]
	,	[STEP_ID]
	,	[RUN_STATUS]
	...
	IF NOT EXISTS (
		SELECT 
			TOP 1 *
		FROM [dbo].[QUEUE_JOBS_TO_LAUNCH_STAGING]
	)
	BEGIN
	...
	,	s.[step_id]
	,	s.[run_status]
	FROM [dbo].[QUEUE_JOBS_TO_LAUNCH_STAGING] s
